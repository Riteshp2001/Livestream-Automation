...
def search_and_download_meditation_video(used_videos, video_query, quality='medium'):
    ...
    if response.status_code == 200:
        ...
        for hit in data['hits']:
            video_url = hit['videos'].get(quality, hit['videos']['medium'])['url']
            if video_url not in used_videos:
                print(f"Video found ({quality}): {video_url}")
